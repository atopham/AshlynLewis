...
def editSingleStudent(request, byu_id):
    employee = StudentEmployee.objects.get(byu_id = byu_id)
    context = {
        "employee":employee,
    }
...
def editSingleSupervisor(request, supervisor_id):
    supervisor = Supervisor.objects.get(supervisor_id=supervisor_id)
    context = {
        "supervisor":supervisor
    }
    return render(request, "manageis/editsupervisor.html", context)

def updateSingleSupervisor(request):
    if request.method == "POST":
        supervisor_id = request.POST['supervisor_id']
        supervisor = Supervisor.objects.get(supervisor_id=supervisor_id)
        supervisor.supervisor_first = request.POST['supervisor_first']
        supervisor.supervisor_last = request.POST['supervisor_last']
        supervisor.save()

    return firetablePageView(request)

def deleteSupervisor(request, supervisor_id):
    supervisor = Supervisor.objects.get(supervisor_id=supervisor_id)
    supervisor.delete()
    return firetablePageView(request)
